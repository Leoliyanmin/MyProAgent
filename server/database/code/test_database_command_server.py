...
class ServerCommandTestRunner:
    def __init__(self) -> None:
        ...
        self.db_path = self.db_dir / "test_server.db"
    ...
    def _has_table_data(self, table: str) -> bool:
        ...
        if table == "code":
            return len(db.list_codes_by_user(user_id, db_path=self.db_path)) > 0
        if table == "category":
    ...
    def run_get(self, table: str) -> None:
        ...
        if table == "user":
            ...
            self._expect(row["user_password_hash"] == user["user_password_hash"], "user_password_hash mismatch")
            ...
            rows = db.list_accounts_by_user(user_id, db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_accounts_by_user should contain at least one row")
            result = rows
        elif table == "code":
            row = db.get_code_by_context(self.data["code"]["code_context"], db_path=self.db_path)
            self._expect(row is not None, "get_code_by_context should return one row")
            rows = db.list_codes_by_user(user_id, db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_codes_by_user should contain at least one row")
            result = {"get_code_by_context": row, "list_codes_by_user": rows}
        elif table == "category":
            rows = db.list_categories_by_user(user_id, db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_categories_by_user should contain at least one row")
            result = rows
        elif table == "data":
            rows = db.list_data_by_user(user_id, db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_data_by_user should contain at least one row")
            result = rows
        elif table == "schedule":
            rows = db.list_schedule_by_user(user_id, db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_schedule_by_user should contain at least one row")
            result = rows
        elif table == "session":
            rows = db.list_sessions_by_user(user_id, db_path=self.db_path)
