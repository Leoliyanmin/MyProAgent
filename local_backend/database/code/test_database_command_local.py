...
class LocalCommandTestRunner:
    ...
    def _has_table_data(self, table: str) -> bool:
        ...
        if table == "data":
            return len(db.list_data(db_path=self.db_path)) > 0
        if table == "schedule":
            return len(db.list_schedule(db_path=self.db_path)) > 0
        if table == "session":
            return len(db.list_sessions(db_path=self.db_path)) > 0
        if table == "chat":
    ...
    def run_get(self, table: str) -> None:
        ...
        if table == "user":
            ...
            rows = db.list_data(db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_data should contain at least one row")
            user_id = rows[0]["user_id"]
            ...
            rows = db.list_schedule(db_path=self.db_path)
            self._expect(len(rows) >= 1, "list_schedule should contain at least one row")
            user_id = rows[0]["user_id"]
            ...
            self._expect(len(rows) >= 1, "list_sessions should contain at least one row")
            user_id = rows[0]["user_id"]
...
def main() -> int:
    ...
    parser.add_argument(
        "--table",
        default="all",
        help="Table to test: all or one table name (user, users, user_match_profile, match_result, sync_state, account, category, data, schedule, session, chat).",
    )
