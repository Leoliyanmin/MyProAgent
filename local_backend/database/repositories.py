...
class AgentChatRepository:
    def create_chat_message(self, chat_data: dict):
        db = SessionLocal()
        try:
            db_chat = AgentChat(
    ...
    def get_chat_history_by_session(self, session_id: str):
        db = SessionLocal()
        try:
            return db.query(AgentChat).filter(AgentChat.session_id == session_id).order_by(AgentChat.created_at).all()
        except Exception as e:
    ...
    def get_chat_history_by_user(self, user_id: int):
        db = SessionLocal()
        try:
            return db.query(AgentChat).filter(AgentChat.user_id == user_id).order_by(AgentChat.created_at).all()
        except Exception as e:
            print(f"Error getting chat history by user: {e}")
            return []
        finally:
            db.close()
