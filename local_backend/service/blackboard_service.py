...
class BlackboardService:
    ...
    def test_cookie_validity(self, cookies_str: str) -> Dict:
        """测试Cookie有效性（不绑定）
        ...
        Returns:
            测试结果，包含Cookie是否有效、是否包含JSESSIONID等信息
        """
        try:
            ...
            try:
                ...
                logger.error("Cookie格式错误，不是有效的JSON")
                return {'success': False, 'message': 'Cookie格式错误，不是有效的JSON'}
            
            logger.info(f"解析到的Cookie: {list(cookies_dict.keys())}")
            ...
            if 'JSESSIONID' not in cookies_dict:
                return {
                    'success': False, 
                    'message': 'Cookie中不包含JSESSIONID',
                    'has_jsessionid': False,
                    'has_s_session_id': 's_session_id' in cookies_dict,
                    'cookie_keys': list(cookies_dict.keys()),
                    'missing_required': missing_required
                }
            
            if 's_session_id' not in cookies_dict:
                return {
                    'success': False, 
                    'message': 'Cookie中不包含s_session_id',
                    'has_jsessionid': True,
                    'has_s_session_id': False,
                    'cookie_keys': list(cookies_dict.keys()),
                    'missing_required': missing_required
                }
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        logger.warning(f"连接失败，重试第 {attempt + 1} 次: {str(e)}")
                        continue
                    else:
                        raise e
            ...
            # 检查是否被重定向到CAS登录页
            is_valid = 'cas.sustech.edu.cn' not in test_response.url
            ...
            if is_valid:
                ...
                try:
                    ...
                    if scrape_result['success']:
                        course_count = len(scrape_result.get('courses', []))
                        return {
                            'success': True,
                            'message': 'Cookie有效，可以成功访问Blackboard',
                            'has_jsessionid': True,
                            'cookie_keys': list(cookies_dict.keys()),
                        ...
                        return {
                            'success': True,
                            'message': 'Cookie有效，但爬取课程数据失败',
                            'has_jsessionid': True,
                            'cookie_keys': list(cookies_dict.keys()),
                            'response_status': test_response.status_code,
                            'scrape_error': scrape_result.get('message', '')
                        }
                except Exception as e:
                    logger.error(f"爬取课程数据失败: {str(e)}")
                    return {
                        'success': True,
                        'message': 'Cookie有效，但爬取课程数据时出错',
                        'has_jsessionid': True,
                        'cookie_keys': list(cookies_dict.keys()),
                        'response_status': test_response.status_code,
                        'error': str(e)
                ...
                return {
                    'success': False,
                    'message': 'Cookie已失效，被重定向到CAS登录页',
                    'has_jsessionid': True,
                    'cookie_keys': list(cookies_dict.keys()),
                    'response_status': test_response.status_code,
                    'redirected_to_cas': True
                }
            ...
            return {
                'success': False, 
                'message': '请求超时，请检查网络环境或稍后重试',
                'error_type': 'timeout',
                'has_jsessionid': True
            }
        except Exception as e:
            logger.error(f"测试Cookie有效性失败: {str(e)}")
