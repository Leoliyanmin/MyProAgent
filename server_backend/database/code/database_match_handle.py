...
try:
    from .database_match_operations import ServerMatchOperations
except ImportError:
    from database.code.database_match_operations import ServerMatchOperations
...
class ServerMatchHandle:
    """Server database-layer entry for profile and match result storage."""

    def __init__(self) -> None:
        self.operations = ServerMatchOperations()
    ...
    def upsert_profile(
        ...
        try:
            ...
            if message.startswith("User not found"):
                return _error(404, "MATCH_USER_NOT_FOUND", message)
    ...
    def set_profile_open(self, user_id: str, is_open: Any) -> dict[str, Any]:
        ...
        try:
            profile = self.operations.set_profile_open(user_id=user_id, is_open=is_open, touch_sync_state=True)
            return {
                "ok": True,
                "status": 200,
                "data": profile,
    ...
    def delete_profile(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            deleted = self.operations.delete_profile(user_id=user_id, touch_sync_state=True)
            if not deleted:
                return _error(404, "MATCH_PROFILE_NOT_FOUND", f"Profile not found: {user_id}")
            return {
                "ok": True,
    ...
    def replace_match_results(
        ...
        if not isinstance(matches, list):
            return _error(400, "MATCH_INVALID_ARGUMENT", "matches must be list")

        try:
            result = self.operations.replace_match_results(
                user_id=user_id,
                matches=matches,
            ...
            state = self.operations.get_sync_state(user_id)
            response_data: dict[str, Any] = dict(result)

            if state is not None:
                version = int(state.get("user_version") or 1)
                last_synced_at = state.get("user_last_synced_at")
                data_updated_at = state.get("user_data_updated_at")
                response_data["server_user_version"] = version
                response_data["server_user_last_synced_at"] = last_synced_at
                response_data["server_user_data_updated_at"] = data_updated_at
                response_data["user_version"] = version
                response_data["user_last_synced_at"] = last_synced_at
                response_data["user_data_updated_at"] = data_updated_at
            ...
            return _error(500, "MATCH_INTERNAL_ERROR", f"replace match results failed: {exc}")
    ...
    def handle(self, action: str, payload: Any = None) -> dict[str, Any]:
        ...
        if action_name in {"replace_match_results", "save_match_results"}:
            if not isinstance(payload, dict):
                return _error(400, "MATCH_INVALID_ARGUMENT", "payload must be object")
            return self.replace_match_results(
                user_id=str(payload.get("user_id") or ""),
